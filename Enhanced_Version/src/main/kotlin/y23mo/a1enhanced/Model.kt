...
class Model {
    ...
    var hmarkView:HighestMark? = null;
    ...
    private fun sort() {
        ...
        else if (sort == "Course name(alphabetically)") {
            displayEntry = displayEntry.sortedBy { it.getTitle() }.toMutableList();
        }
    ...
    private fun calcStatus() {
        ...
        displayEntry.forEach {
            if (it.getGrade() != "WD") {
                ...
                courses++;
                if (it.getGrade().toInt() > hmark) {
                    hmark = it.getGrade().toInt();
                }
